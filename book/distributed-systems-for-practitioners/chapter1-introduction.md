...
- 并发执行：多个操作在不同结点同时执行，相互影响，保证不了隔离性，会导致意想不到的情况
...
要实现去重方法的话，我们需要对系统的两端都能掌控，也即是在发送端要为每条消息都生成一个全局唯一ID，接收端收到这条消息之后要能通过这个全局唯一ID去判断这条消息是否已经处理过了。如果没处理，则进入处理流程；如果已经处理过了，那么直接忽略。这样我们就保证了消息的**仅仅一次处理**。
